...
async function testRealDilithium() {
  ...
  console.log(`✅ Secret key length: ${secretKey.length} bytes (expected: 4000)`);

  if (publicKey.length !== 1952) {
    throw new Error(`Invalid public key length: ${publicKey.length}`);
  }
  if (secretKey.length !== 4000) {
  ...
  console.log(`✅ Signature length: ${signature.length} bytes (expected: 3293)`);
  ...
  const invalidSignature = new Uint8Array(3293);
